#!/usr/bin/env python3
"""
Standalone Historical PTM ETL Script
...
# Configure logging
logging.basicConfig(
    filename='ptm_historical_etl.log',
    level=logging.INFO,
...
api_url = config.get('api', 'ptm_url', fallback='https://akanksha.edustems.com/getStudentAttendancePTM.htm')
...
# Database table creation SQL for PTM data
CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS ptm_attendance_data (
    ...
    ptm_attendance_data_unique_key VARCHAR(500) UNIQUE
);
"""

def setup_database():
    """Create the ptm_attendance_data table if it doesn't exist"""
    try:
        # Connect to MySQL
        ...
        logging.info("Creating ptm_attendance_data table if it doesn't exist...")
        ...
        # Verify table exists
        cursor.execute("SHOW TABLES LIKE 'ptm_attendance_data'")
...
def generate_ptm_unique_key(row, academic_year, month):
    """Generate unique key for PTM data"""
    return f"{academic_year}|{month}|{row['school_name']}|{row['grade_name']}|{row['student_id']}"

def validate_ptm_record(record):
    """Validate PTM record data"""
    required_fields = ['school_name', 'grade_name', 'student_id', 'student_name', 'academic_year', 'month', 'date']
...
def insert_data_to_mysql(df, stats):
    ...
    try:
        ...
        insert_query = """
        INSERT INTO ptm_attendance_data 
        (school_name, grade_name, division_name, course_name, student_id, student_name, 
         ...
         attendance_percentage, ptm_attendance_data_unique_key)
        ...
        for _, row in df.iterrows():
            try:
                cursor.execute(insert_query, (
                    ...
                    row['total_no_of_ptm'], row['present_ptm'], 
                    row['attendance_percentage'], row['ptm_attendance_data_unique_key']
                ))
                conn.commit()
                inserted_count += 1
            except mysql.connector.Error as err:
                if err.errno == 1062:  # Duplicate entry error
                    duplicate_count += 1
                    logging.warning(f"Duplicate entry skipped: {row['ptm_attendance_data_unique_key']}")
                else:
                    error_count += 1
                    logging.error(f"Database error for row {row['ptm_attendance_data_unique_key']}: {err}")
                    raise
...
def process_historical_data():
    """Process historical PTM data for multiple academic years"""
    logging.info("=== Starting Historical PTM ETL Process ===")
    start_time = datetime.now()
    
    # Setup database
    setup_database()
    
    # Define academic years to process (adjust as needed)
    ...
    total_stats = {
        ...
        'total_errors': 0,
        'academic_years_processed': 0,
        'months_processed': 0
    }
    
    for academic_year in academic_years:
        logging.info(f"Processing academic year: {academic_year}")
        year_stats = {
            'records_fetched': 0,
            'records_inserted': 0,
        ...
        for month in months:
            logging.info(f"Processing {month} {academic_year}")
            
            # Fetch data from API
            df = fetch_data_from_api(academic_year, month)
            
            if df.empty:
                logging.info(f"No data found for {month} {academic_year}")
                continue
            ...
            df['ptm_attendance_data_unique_key'] = df.apply(
                lambda row: generate_ptm_unique_key(row, academic_year, month), axis=1
            )
            ...
            # Update statistics
            year_stats['records_fetched'] += len(df)
            year_stats['records_inserted'] += stats.get('new_records_inserted', 0)
            year_stats['duplicates_skipped'] += stats.get('duplicate_records_skipped', 0)
            year_stats['errors'] += stats.get('error_records', 0)
            
            total_stats['months_processed'] += 1
        
        # Log year statistics
        logging.info(f"Year {academic_year} completed: {year_stats}")
        total_stats['academic_years_processed'] += 1
        total_stats['total_records_fetched'] += year_stats['records_fetched']
        total_stats['total_records_inserted'] += year_stats['records_inserted']
        total_stats['total_duplicates_skipped'] += year_stats['duplicates_skipped']
    ...
    logging.info("=== Historical PTM ETL Process Completed ===")
    logging.info(f"Total processing time: {processing_time:.2f} seconds")
    logging.info(f"Academic years processed: {total_stats['academic_years_processed']}")
    logging.info(f"Months processed: {total_stats['months_processed']}")
    logging.info(f"Total records fetched: {total_stats['total_records_fetched']}")
    logging.info(f"Total records inserted: {total_stats['total_records_inserted']}")
    logging.info(f"Total duplicates skipped: {total_stats['total_duplicates_skipped']}")
    logging.info(f"Total errors: {total_stats['total_errors']}")
